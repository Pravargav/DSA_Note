...
class FrogJumpDP {
    ...
    // Function to compute minimum energy cost
    static int findMinEnergy(int index, int[] heights, int[] memo) {
        ...
        // Jump from previous step
        int jumpOneStep = findMinEnergy(index - 1, heights, memo)
                + Math.abs(heights[index] - heights[index - 1]);

        // Jump from two steps before
        int jumpTwoSteps = Integer.MAX_VALUE;
        if (index > 1) {
            jumpTwoSteps = findMinEnergy(index - 2, heights, memo)
                    + Math.abs(heights[index] - heights[index - 2]);
        }

        // Store and return result
        return memo[index] = Math.min(jumpOneStep, jumpTwoSteps);
    }
    ...
    public static void main(String[] args) {
        ...
        int minEnergy = findMinEnergy(n - 1, heights, memo);
