...
-> note: {3, -1, -5, 2, 5, -9} return 1 as index==nums.length, max of (-9, 1) = 1, max(5*1,1*1)= 5*1 , max(2*5*1,1*5*1)=2*5*1, max(-5*2*5*1,1*2*5*1)=1*2*5*1,

max(-1*1*2*5*1,1*1*2*5*1)=1*1*2*5*1, max(3*1*1*2*5*1,1*1*1*2*5*1)=3*1*1*2*5*1 = 30
````
